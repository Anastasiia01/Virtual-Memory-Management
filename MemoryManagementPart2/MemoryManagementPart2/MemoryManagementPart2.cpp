...
//getPageNumber
//frame_index = swapPage_Optimal(frame_index, address_list, PC(pagecounter), length(of address_list) 3);
int swapPage_Optimal(int frame_index_param, int *addressList, int current, int addressLength)
{
	...
	if (frame_index == NOT_FOUND)//need to swap
		...
		{
			TLBpage = getPageNumber_from_TLB(j);
			for (PC = addressIndex; PC < addressLength;PC++) {
				page = getPageNumber(addressList[PC]);
				if (page == TLBpage) {
					pageFound = true;
